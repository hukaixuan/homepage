...
def get_catalog():
    return {
            'users_url': url_for('api.get_users', _external=True),
            # 'classes_url': url_for('api.get_classes', _external=True),
            # 'registrations_url': url_for('api.get_registrations',_external=True)
            }
...
from . import users, posts, labels, comments, sites
