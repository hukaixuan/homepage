...
export default {
    ...
    mounted: function(){
        ...
        if(this.$route.query.id){
            this.$http.get('http://127.0.0.1:5000/api/v1/posts/'+this.$route.query.id).then(
                respone => {
                    this.articleTitle = respone.body.title,
                    this.list.push({
                        tag_id: respone.body.label_id
                    }),
    ...
    methods: {
        ...
        delectArticles: function(){
            var options = {
                url: 'http://127.0.0.1:5000/api/v1/posts/'+this.$route.query.id,
                method: 'DELETE',
                headers: 
                {
                    Authorization: 'Basic '+window.btoa(getCookie('token'))  // 要用base64进行编码后发送
                }
            }
