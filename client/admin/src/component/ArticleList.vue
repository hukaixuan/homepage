        ...
        <div class="article-list">
            ...
            <ul v-articleListHeight>
                <li v-for="item in articleList">
                    ...
                    <!-- <h3>{{item.title}}</h3> -->
                    <p>{{new Date(item.post_time).format('yyyy-MM-dd hh:mm:ss')}}
                        <span class="label-item">{{item.label_name}}</span>
                    </p>
                </li>
            </ul>
        </div>
...
export default{
    ...
    created: function(){
        ...
        var param = localStorage.getItem("label_id")
        if(param){
            localStorage.removeItem("label_id")
            this.$http.get('http://127.0.0.1:5000/api/v1/labels/'+param+'/posts?expand=1').then(
                respone => {
    ...
    methods: {
        fetchData: function(){
            this.$http.get('http://127.0.0.1:5000/api/v1/posts?page=1&expand=1').then(
                respone => this.articleList = respone.body.posts,
                respone => console.log(respone.body.posts)
            )
        },
        // 文章编辑页路由
        addArticle: function(){
        ...
        refreshArticleList: function(){
            // 加一个随机的refresh字段来生成不同的URL，防止浏览器缓存不进行刷新
            this.$http.get('http://127.0.0.1:5000/api/v1/posts?page=1&expand=1&refresh='+Math.random()).then(
                respone => this.articleList = respone.body.posts,
                respone => console.log(respone.body.posts)
            )
        },
